...
mod tests {
    ...
    fn splits_tokens_at_known_target_names() {
        ...
        let sections =
            parse_grouped_targets(&tokens, &["deb", "rpm", "brew"]).expect("sections should parse");

        assert_eq!(sections.len(), 3);
        assert_eq!(sections[0].name, "deb");
        assert_eq!(sections[0].args, [os("--prefix"), os("apt/gmutils")]);
        assert_eq!(sections[1].name, "rpm");
        assert_eq!(sections[1].args, [os("--prefix"), os("rpm/gmutils")]);
        assert_eq!(sections[2].name, "brew");
    ...
    fn rejects_empty_target_list() {
        let error =
            parse_grouped_targets(&[], &["deb", "rpm"]).expect_err("empty target list should fail");
    ...
    fn rejects_tokens_before_first_target() {
        let tokens = [os("--prefix"), os("apt/gmutils"), os("deb")];

        let error = parse_grouped_targets(&tokens, &["deb", "rpm"])

...
type ClientNameCache = Vec<(Weak<DynConnectionState>, Option<String>)>;
...
impl AccessControlState {
    ...
    async fn client_name(&self, connection: &Arc<DynConnectionState>) -> Option<String> {
        ...
        match connection.remote_authority().await {
            Ok(authority) => {
                let client_name = authority.map(|authority| authority.name().to_owned());

...
pub async fn file_handle(
    ...
    let file_path = if let Some(alias) = location.alias() {
        static_file_path(alias.as_ref(), &loc.remaining)
    } else if let Some(root) = location.root() {
        static_file_path(root.as_ref(), req.uri().path())
    } else {
